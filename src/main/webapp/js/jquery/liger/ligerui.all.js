...
(function ($)
{
    ...
    //dialog 图片文件夹的路径 预加载
    l.DialogImagePath = "../../skin/jquery/liger/Aqua/images/win/";
...
//            this.loadData(this.options.data);
        ...
        _getCellContent: function (rowdata, column)
        {
            ...
            if(column.name.indexOf(".")>0){
                var columns = column.name.split(".");
                for(var i=0;i<columns.length;i++){
                    if(i == 0){
                        value = rowdata[columns[i]];
                    }else{
                        value = value?value[columns[i]]:"";
                    }
                }
            }
            var g = this, p = this.options;
...
(function ($)
{
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        append: function (parentNode, newdata, nearNode, isAfter)
        {
            ...
            //added by tcg ******************************************
            treeitem.attr("loaded",true);
            for(var j=0;j<g.options.callBackParams.length;j++){
                treeitem.attr(g.options.callBackParams[j],newdata[0][g.options.callBackParams[j]]);
            }
            //******************************************
...
//            var hasChildren = $("> ul", treeitem).length > 0;
            var hasChildren =  newdata.isParent;
            if (!hasChildren)
            {
        ...
        _getTreeHTMLByData: function (data, outlineLevel, isLast, isExpand)
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                var isExpandCurrent = false;
                var o = data[i];
                if (o.isexpand == true || o.isexpand == "true") isExpandCurrent = true;
            ...
            g.tree.click(function (e)
            {
                ...
                else if (treeitembtn.hasClass("l-expandable-close") && (!p.btnClickToToggleOnly || clickOnTreeItemBtn))
                {
                    ...
                    if (p.slide)
                    {
                        ...
                        //added by tcg ******************************************
                        if(!treeitem.attr("loaded")){
                            var params = {};
                            for(var j=0;j<g.options.callBackParams.length;j++){
                                params[g.options.callBackParams[j]] = treeitem.attr(g.options.callBackParams[j]);
                            }
                            g.loadData(treeitem,g.options.url,params);
                        }
                        //*****************
                    }
                    else
