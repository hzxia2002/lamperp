...
$.Window = (function()  {
	...
	function constructor(caller, options) {
		...
		var options = $.extend({
            ...
            userIframe : false  // 是否适应Iframe 默认为false
		}, options);
		...
		function initialize(instance) {
			...
			if( options.url != null && $.trim(options.url) != "" ) {
                ...
                // 判断是够显示iFrame
                if(options.userIframe != null && options.userIframe == true) {
                    // append iframe html
                    ...
                    $.ajax({
                        ...
                        error: function(err) {
                            setContent('获取内容失败:' + err.responseText + '  Status: ' + err.status);
                            frame.css({
                                overflow: options.scrollable? "auto":"hidden"
                            });
                        }
